...
- **The `bash` skill executes whatever shell command the model emits.** That is its job. If you point the agent at an untrusted model or paste an untrusted prompt, the model can run arbitrary commands as the user that launched `pym`. Treat the agent like a shell session — don't run it as root, don't run it against random servers, and review the prompt and tool calls before approving destructive actions.
