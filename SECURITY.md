...
- **The `bash` skill executes whatever shell command the model emits.** That is its job. If you point the agent at an untrusted model or paste an untrusted prompt, the model can run arbitrary commands as the user that launched `pi`. Treat the agent like a shell session — don't run it as root, don't run it against random servers, and review the prompt and tool calls before approving destructive actions.
...
- **Custom skills that hit the network** can be a vector. Validate inputs and prefer narrow APIs over `bash`.
...
- HTML escape misses in `pi.session.export_html` that allow an attacker who controls part of a session to inject script tags into the rendered transcript.
- RPC parse handling in `pi.rpc` that lets a malicious stdin stream crash the process or escape the command dispatcher.
- Path traversal or injection in the `read` / `edit` / `write` skills beyond the usual "the LLM asked for it" surface — i.e., something the model can accidentally trigger via well-formed-looking arguments.
- Session loader (`pi.persistence.Session.load`) parsing issues that let a crafted JSONL file crash or hang the process.
