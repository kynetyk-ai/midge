...
Our internal `AgentEvent` (in `src/pi/agent.py` and `src/pi/client.py`) maps cleanly to pi-mono's wire events:
...
1. Process spawns with `pi --rpc` (or however we wire it; an `examples/rpc_agent.py` is the simplest start).
