...
- `src/pi/session.py` (or `src/pi/export.py` — name TBD; if we anticipate session save/load in Phase 3 the file should be `session.py` and the export function lives there as a sibling to save/load):
  - `def export_html(messages: list[Message], *, title: str = "pi session", model: str = "") -> str` — returns the full HTML document as a string.
